...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of AceLabs, please download the latest version! [AceLabs Editor](https://voideditor.com/download-beta)!'
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `AceLabs Error: There was an error checking for updates. If this persists, please get in touch or reinstall AceLabs [here](https://voideditor.com/download-beta)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
const performVoidCheck = async (
	...
	metricsService.capture(`AceLabs Update ${metricsTag}: Checking...`, {})
	const res = await voidUpdateService.check(explicit)
	if (!res) {
		const notifController = notifyErrChecking(notifService);
		metricsService.capture(`AceLabs Update ${metricsTag}: Error`, { res })
		return notifController
	}
	else {
		if (res.message) {
			const notifController = notifyUpdate(res, notifService, updateService)
			metricsService.capture(`AceLabs Update ${metricsTag}: Yes`, { res })
			return notifController
		}
		else {
			metricsService.capture(`AceLabs Update ${metricsTag}: No`, { res })
			return null
		}
	}
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'AceLabs: Check for Updates'),
		});
	}
